...
mod test {
    ...
    fn test_generate_secret_manifests_single_no_vault_directory() {
        ...
        assert_eq!(
            ...
            "Unable to read secret \"/path/to/nonexistent/vault/kvstore/secret.yaml\""
        );
    }
    ...
    fn test_generate_rbac_manifests_single_no_vault_directory() {
        ...
        assert_eq!(
            manifests.unwrap_err().to_string(),
            "Unable to read access control rules for \"user\" on \"/path/to/nonexistent/vault/access_control/user\""
        );
    }
